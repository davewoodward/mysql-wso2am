<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="xs">
   ...
   <xsl:template match="datasource[name='WSO2AM_DB']/definition/configuration/url">
      <xsl:copy>
         <xsl:value-of select="concat(concat(concat(concat(concat(concat('jdbc:mysql://', $sqlhost), ':'), $sqlport), '/'), $apidb), '?autoReconnect=true&amp;relaxAutoCommit=true')"/>
      </xsl:copy>
   </xsl:template>
   ...
   <xsl:template match="datasource[name='WSO2UM_DB']/definition/configuration/url">
      <xsl:copy>
         <xsl:value-of select="concat(concat(concat(concat(concat(concat('jdbc:mysql://', $sqlhost), ':'), $sqlport), '/'), $userdb), '?autoReconnect=true&amp;relaxAutoCommit=true')"/>
      </xsl:copy>
   </xsl:template>
   ...
   <xsl:template match="datasource[name='WSO2REG_DB']/definition/configuration/url">
      <xsl:copy>
         <xsl:value-of select="concat(concat(concat(concat(concat(concat('jdbc:mysql://', $sqlhost), ':'), $sqlport), '/'), $regdb), '?autoReconnect=true&amp;relaxAutoCommit=true')"/>
      </xsl:copy>
   </xsl:template>
